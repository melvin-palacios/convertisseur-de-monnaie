...
window.maxsize(315, 375)
window.minsize(315, 375)
...
def calcul():
    ...
    if valeur.get() != "":
        if variable1.get() == "EUR" and variable2.get() == "JPY":
            result = round(int(valeur.get()) / taux_de_change['EUR_JPY'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        elif variable1.get() == "EUR" and variable2.get() == "USD":
            result = round(int(valeur.get()) / taux_de_change['EUR_USD'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        elif variable1.get() == "USD" and variable2.get() == "EUR":
            result = round(int(valeur.get()) / taux_de_change['USD_EUR'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        elif variable1.get() == "USD" and variable2.get() == "JPY":
            result = round(int(valeur.get()) / taux_de_change['USD_JPY'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        elif variable1.get() == "JPY" and variable2.get() == "EUR":
            result = round(int(valeur.get()) / taux_de_change['JYP_EUR'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        elif variable1.get() == "JPY" and variable2.get() == "USD":
            result = round(int(valeur.get()) / taux_de_change['JPY_USD'],5)
            valeur2.insert(0, str(result))
            valeur2.place(x=45, y=290)
        else:
...
save_button = Button(frame1, text="  sauvegarder  ", font=("Arial",15), bg='white', fg='#2A77EA')
...
# frame 2
label_title_historique = Label(frame2,text="En construction", font=("Arial",15), bg='#2A77EA',fg='White')
